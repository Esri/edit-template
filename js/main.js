 ...
 */
define(["dojo/_base/declare", "dojo/Deferred", "dojo/has", "dojo/_base/lang", "dojo/_base/kernel", "dojo/_base/Color", "dojo/_base/array", "dojo/on", "dijit/registry", "esri/arcgis/utils", "esri/lang", "dojo/dom-geometry", "dojo/dom-style", "dojo/query", "dojo/dom-construct", "dojo/dom-class", "application/Drawer", "esri/layers/FeatureLayer", "esri/dijit/editing/Editor", "esri/toolbars/edit", "esri/tasks/query", "application/SearchSources", "dojo/domReady!"], function (
  declare, Deferred, has, lang, kernel, Color, array, on, registry, arcgisUtils, esriLang, domGeometry, domStyle, query, domConstruct, domClass, Drawer, FeatureLayer, Editor, Edit, esriQuery, SearchSources) {
  return declare(null, {
    config: {},
    editor: null,
    editable: false,
    ...
    _createWebMap: function (itemInfo) {
      ...
      arcgisUtils.createMap(itemInfo, "mapDiv", options).then(lang.hitch(this, function (response) {
        ...
        on(this.map.infoWindow, "show", lang.hitch(this, this._updatePopup));
        on(this.map.infoWindow, "restore", lang.hitch(this, this._updatePopup));
    ...
    _updatePopup: function (e) {
      ...
      this._updateLinkVisibility(this.map.infoWindow.getSelectedFeature(), this.popupLink);
    ...
    _setupEditablePopup: function () {
      ...
      if (this.editableLayers) {
        this.editableLayers.forEach(function (layerInfo) {
          var fl = layerInfo && layerInfo.featureLayer;
          if (fl) {
            layerInfo.featureLayer.on("edits-complete", lang.hitch(this, function (edits) {
              var hasEdits = edits.adds.length;
              if (hasEdits) {
                var added = edits.adds[0];
                var query = new esriQuery();
                query.objectIds = [added.objectId];
                fl.queryFeatures(query, lang.hitch(this, function (featureSet) {
                  var feature = featureSet.features && featureSet.features[0];
                  if (feature) {
                    var geometry = feature.geometry;
                    var location = getPointFromGeometry(geometry);
      ...
      on(link, "click", lang.hitch(this, function () {
        ...
        if (selectedFeature && selectedFeature._layer) {
          if (typeof selectedFeature._layer.isEditable === "function") {
            this._destroyEditor();
            this._createEditor().then(lang.hitch(this, function () {
              ...
              this.map.infoWindow.show(location);
              this.editor.editToolbar.activate(Edit.MOVE, selectedFeature);
            }));
          }
        }
      }));
      ...
      on(this.map.infoWindow, "selection-change", lang.hitch(this, function () {
        ...
        this._updateLinkVisibility(selected, link);
      }));

    },
    _updateLinkVisibility: function (selected, link) {
      if (selected && selected._layer) {
        if (typeof selected._layer.isEditable === "function" && selected._layer.isEditable()) {
          // show editable link
    ...
    _createEditor: function () {
      ...
      if (this.editable) {
        ...
        domConstruct.place(this.editor.domNode, document.getElementById("editorDiv"));
        ...
        if (this.config.customLayout !== "default") {
          this.editor.on("load", function () {
            query(".esriDrawingToolbar").forEach(function (node) {
              domConstruct.place(node, document.getElementById("editorDiv"), "before");
            });
          });
        }
        this.editor.startup();
    ...
    _disableClustering: function (layer) {
      if (layer) {
        if (layer.isFeatureReductionEnabled && layer.disableFeatureReduction) {
        ...
        if (layer && layer.layerObject) {
          ...
          if (eLayer instanceof FeatureLayer) {
            if (eLayer.isEditable()) {
              ...
              this._disableClustering(eLayer);
            } else {
